...
export function useAdminUsers(search?: string, role?: string, page: number = 1, limit: number = 50, sortBy: string = 'pending_first') {
  const { token } = useAuth();
  return useQuery({
    queryKey: ['admin-users', search, role, page, limit, sortBy],
    queryFn: async () => {
      const response = await adminApi.getUsers(token!, search, role, page, limit, sortBy);
      // Backend now returns { users, pagination } inside response.data
      return response.data;
    },
    enabled: !!token,
  });
