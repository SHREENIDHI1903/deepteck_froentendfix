...
export function Navbar() {
  ...
  const buyerNotifs = (notifCounts?.buyer?.projects || 0) + (notifCounts?.buyer?.contracts || 0);
  const expertNotifs = (notifCounts?.expert?.proposals || 0) + (notifCounts?.expert?.invitations || 0) + (notifCounts?.expert?.contracts || 0);
                      ...
                      <div className="p-2">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            {!isBuyer && <NavBadge count={buyerNotifs} />}
                          </div>
                          {isBuyer ? (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            {!isExpert && <NavBadge count={expertNotifs} />}
                          </div>
                          {isExpert ? (
                            <span className="text-[10px] font-semibold text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-full">
                              {"Active"}
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <div className="space-y-1">
                    ...
                    {isExpert && (
                      <>
                        ...
                        <Link to="/proposals" className={`flex items-center gap-3 p-3 rounded-xl hover:bg-muted text-sm font-medium transition-colors ${!canUseExpertApp ? 'opacity-50 pointer-events-none' : ''}`} onClick={() => setMobileMenuOpen(false)}>
                          <Inbox className="h-5 w-5 text-muted-foreground" /> <span className="flex items-center">Proposals/Invitations<NavBadge count={notifCounts?.invitations ?? notifCounts?.proposals} /></span>
                        </Link>
                        ...
                        <Link to="/experts/leaderboard" className={`flex items-center gap-3 p-3 rounded-xl hover:bg-muted text-sm font-medium transition-colors ${!canUseExpertApp ? 'opacity-50 pointer-events-none' : ''}`} onClick={() => setMobileMenuOpen(false)}>
                          <Search className="h-5 w-5 text-muted-foreground" /> Leaderboard
                        </Link>
                      </>
                    )}
                    <button
                      className={`flex items-center gap-3 p-3 rounded-xl hover:bg-muted text-sm font-medium transition-colors w-full text-left ${!canUseExpertApp && isExpert ? 'opacity-50 pointer-events-none' : ''}`}
                      onClick={() => {
                        if (canUseExpertApp || !isExpert) {
                          navigate("/messages");
                          setMobileMenuOpen(false);
                    ...
                    <Link to="/support" className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted text-sm font-medium transition-colors" onClick={() => setMobileMenuOpen(false)}>
                      <LifeBuoy className="h-5 w-5 text-muted-foreground" /> Support
                    </Link>
                  </div>
