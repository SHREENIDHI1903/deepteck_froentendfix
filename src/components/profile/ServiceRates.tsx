...
function RateInput({
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Block extremely long inputs (User reported 15 digits!)
        if (val.replace('.', '').length > 8) return;

        // Cap value at 9,999,999 (Approx 1 Crore / 100k USD) to prevent DB overflow and realistic values
        if (parseFloat(val) > 9999999) return;
