...
interface ExtendedProject extends Project {
  ...
  buyer_verified_payment?: boolean;
...
export function ProjectCard({ project, compact = false, context = 'default' }: ProjectCardProps) {
  ...
  const isPaymentVerified = project.buyer?.verified_payment ?? (project as any).buyer_verified_payment ?? false;
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground pt-3 border-t border-border/50">
            {showBuyerInsights && (
              <div className="flex flex-wrap items-center gap-2 w-full">
                {project.buyer_username && (
                  <span className="text-muted-foreground font-medium mr-1">@{project.buyer_username}</span>
                )}

                <div className="flex items-center gap-1" title={isPaymentVerified ? "Payment Verified" : "Payment Unverified"}>
                  <BadgeCheck className={`h-3.5 w-3.5 ${isPaymentVerified ? "text-blue-600 fill-blue-600/10" : "text-muted-foreground"}`} />
                  <span className={`hidden sm:inline ${isPaymentVerified ? "text-foreground" : "text-muted-foreground"}`}>
                    {isPaymentVerified ? "Payment verified" : "Payment unverified"}
                  </span>
                </div>
                ...
                {clientRating > 0 && (
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-muted-foreground">({project.buyer?.review_count || project.buyer_review_count || 0})</span>
                  </div>
                )}
                {clientRating > 0 && <span className="text-muted-foreground/30 mx-1">•</span>}
                ...
                {clientLocation && (
                  <>
                    ...
                    <div className="flex items-center gap-1 text-muted-foreground">
                      ...
                      <span className="truncate max-w-[100px]">{clientLocation}</span>
                    </div>
                  </>
                )}
              </div>
            ...
            {(project.proposal_count !== undefined && project.proposal_count >= 0) && (
              <div className="w-full" title={`${project.proposal_count} proposal(s)`}>
                <span className="font-medium text-foreground">
                  {'Proposals'}: {isOwner ? proposalCountLabel(project.proposal_count) : getProposalRange(project.proposal_count)}
                </span>
