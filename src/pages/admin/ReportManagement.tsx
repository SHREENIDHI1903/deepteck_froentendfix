...
const TableUserCell = ({ userId }: { userId: string }) => {
  ...
  return (
    <div className="flex flex-col">
      <span className="font-medium block text-zinc-900 leading-tight">
        {user ? `${user.first_name} ${user.last_name}` : 'Unknown User'}
      </span>
      <span className="text-[10px] text-zinc-400 font-mono leading-none mt-1 select-all" title="User ID">
        {userId}
      </span>
    </div>
  );
  ...
  const filteredReports = useMemo(() => {
    ...
    return reports.filter((report: any) => {
      const matchesSearch =
        ...
        report.reported_id === searchQuery ||
        report.reporter_id === searchQuery;
  ...
  const columns = [
    {
      ...
      cell: (item: any) => (
        <Badge variant="outline" className={`capitalize ${item.type === 'harassment' ? 'border-red-200 text-red-700 bg-red-50' :
