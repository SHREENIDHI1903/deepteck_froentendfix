...
export default function Financials() {
  ...
  const columns = [
    ...
    {
      header: 'Amount',
      cell: (item: any) => <span className="font-medium">₹{Number(item.amount).toLocaleString('en-IN')}</span>
    },
    {
      header: 'Type',
    ...
    },
    {
      ...
      cell: (item: any) => (
        <span className={`text-xs px-2 py-1 rounded-full capitalize ${item.status === 'processed' ? 'bg-emerald-100 text-emerald-700' :
