...
export default function ClientPublicProfile() {
  ...
  const { data: rawClient, isLoading: isLoadingClient, error } = useClient(id!);



  // Derive client early to use ID in hooks
  ...
  // Use buyer_profile_id if available, otherwise skip fetching until loaded
  // This ensures we get the correct projects for this COMPANY profile, not user ID
  const { data: marketplaceProjects = [], isLoading: isLoadingProjects } = useMarketplaceProjects(
    client?.buyer_profile_id,
    { enabled: !!client?.buyer_profile_id }
  );
  ...
  // Check for 403 (Deactivated/Deleted)
    ...
    return (
      <Layout>
        <AlertDialog open={true}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Account Unavailable</AlertDialogTitle>
              <AlertDialogDescription>
                {(error as any).message || 'This account has been deactivated and is no longer accessible.'}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
  ...
  // Filter projects by buyer_profile_id if available, otherwise fallback to id (user_id) but likely won't match
  const buyerIdToMatch = client.buyer_profile_id || client.id;
  const clientProjects = marketplaceProjects
    .filter((p: any) => p.buyer_id === buyerIdToMatch && p.status === 'open')
    ...
    <Layout>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <ReportDialog
          ...
          reportedId={client.user_id || client.id}
          reportedName={client.username || client.company_name || client.first_name || 'Client'}
          type="user"
        />
              ...
              <CardContent className="p-6">
                <Tabs defaultValue="jobs">
                  <TabsList className="w-full justify-start h-auto flex-wrap gap-2 bg-muted/30 p-1 rounded-xl mb-6">
                    <TabsTrigger value="jobs" className="px-4 py-2 rounded-lg">Open Projects ({clientProjects.length})</TabsTrigger>
                        ...
                        {clientProjects.length > 3 && (
                          <div className="pt-2">
                            <Button
                              variant="outline"
                              className="w-full"
                              onClick={() => navigate(`/marketplace?buyerId=${buyerIdToMatch}`)}
                            >
                              View all projects
                            </Button>
                          </div>
