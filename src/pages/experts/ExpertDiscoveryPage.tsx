...
import { Search, SlidersHorizontal, X, Loader2, UserX, Sparkles, AlertTriangle } from 'lucide-react';
...
const getSavedState = () => {
  ...
  try {
    ...
    // CRITICAL FIX: If the cached data contains the rate limit error, discard it!
    // This prevents the page from getting stuck on the error screen after a refresh.
    if (parsed?.cachedSemanticExperts && (parsed.cachedSemanticExperts as any).__rateLimited) {
      parsed.cachedSemanticExperts = null;
    }
...
export default function ExpertDiscoveryPage() {
  ...
  const isCacheValidForSemantic = useMemo(() => {
    // If the cache is an error object, it's NOT valid.
    if (cachedSemanticExperts && (cachedSemanticExperts as any).__rateLimited) return false;
  ...
  useEffect(() => {
    // CRITICAL FIX: Do NOT save the data if it is the "Rate Limit" error object.
    // This ensures that when the user refreshes, they don't load the error from cache.
    const validSemanticData = (fetchedSemanticExperts && !(fetchedSemanticExperts as any).__rateLimited)
      ? fetchedSemanticExperts
      : (cachedSemanticExperts && !(cachedSemanticExperts as any).__rateLimited)
        ? cachedSemanticExperts
        : null;
    ...
    const stateToSave = {
      ...
      cachedSemanticExperts: validSemanticData // Save only valid data
    };
  ...
  const filteredExperts = useMemo(() => {
    // START CHANGE: Handle 429 object or null
    if (!experts) return [];
    if ((experts as any).__rateLimited) return [];
    // END CHANGE
  ...
  const isRateLimited = useSemanticSearch && experts && (experts as any).__rateLimited;
  const rateLimitMessage = (experts as any)?.message || "Search limit reached.";
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center border rounded-xl bg-orange-50/50 border-orange-200">
                <div className="bg-orange-100 p-3 rounded-full mb-4">
                  <AlertTriangle className="h-6 w-6 text-orange-600" />
                </div>
                <h3 className="text-lg font-semibold text-orange-900 mb-2">
                  Coming Soon: Enhanced AI Search!
                </h3>
                <p className="text-sm text-orange-800/80 max-w-md mb-6 px-4">
                  {rateLimitMessage}
                </p>
                <Button
                  variant="outline"
                  className="border-orange-200 hover:bg-orange-100 text-orange-900"
