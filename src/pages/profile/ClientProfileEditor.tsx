...
export function ClientProfileEditor() {
  ...
  const handleSaveBanner = async (file: File) => {
    if (!token) return;
    try {
      console.log(`[DEBUG_PATH] Frontend Sending Payload. Username: '${formData.username}' Length: ${formData.username.length}`);
  ...
  const handleSaveProfile = async () => {
    if (!formData.first_name || !formData.last_name) {
      return toast({ title: "First name and Last name are required", variant: "destructive" });
    }
    ...
    // Username Validation
    if (formData.username.length < 3) return toast({ title: "Username too short", description: "Minimum 3 characters required", variant: "destructive" });
    if (/^\d+$/.test(formData.username)) return toast({ title: "Invalid Username", description: "Username cannot be purely numeric", variant: "destructive" });
