...
export default function ExpertProfileEditor() {
    ...
    const handle_save = async () => {
        console.log('[DEBUG] Saving Expert Profile Payload:', form_data);
        // Note: Removed blocking validation for service rates so users can save partial updates (e.g., change username) without filling rates.
        ...
        // Username Validation
        if (form_data.username.length < 3) return toast({ title: "Username too short", description: "Minimum 3 characters required", variant: "destructive" });
        if (/^\d+$/.test(form_data.username)) return toast({ title: "Invalid Username", description: "Username cannot be purely numeric", variant: "destructive" });
        ...
        try {
            ...
            try {
                ...
                // Build payload and only include numeric rate fields when they are > 0.
                ...
                const hourly = Number(form_data.avg_hourly_rate || 0)
                const daily = Number(form_data.avg_daily_rate || 0)
                const sprint = Number(form_data.avg_sprint_rate || 0)
                const fixed = Number(form_data.avg_fixed_rate || 0)

                if (hourly > 0) expertUpdate.avg_hourly_rate = hourly
                if (daily > 0) expertUpdate.avg_daily_rate = daily
                if (sprint > 0) expertUpdate.avg_sprint_rate = sprint
                if (fixed > 0) expertUpdate.avg_fixed_rate = fixed

                // Only include completion/status flags if profile appears complete
                if (isComplete) {
                    expertUpdate.is_profile_complete = true
                    if (newStatus) expertUpdate.expert_status = newStatus
                }
