...
export default function MarketplacePage() {
  ...
  const buyerIdFilter = searchParams.get('buyerId') || searchParams.get('Buyer Id') || searchParams.get('Buyer_id') || undefined;

  // Redirect Buyers to their projects page
  ...
  const filteredProjects = activeList.filter((project) => {
    ...
    // Check client details (username, name, company)
    const matchesClient =
      project.buyer_name?.toLowerCase().includes(term) ||
      project.buyer?.company_name?.toLowerCase().includes(term) ||
      project.buyer?.username?.toLowerCase().includes(term) ||
      // Also check the specific buyer_username field we just added for consistency
      (project as any).buyer_username?.toLowerCase().includes(term);

    const matchesSearch = matchesProject || matchesClient;
  ...
  const hasActiveFilters = searchTerm !== '' || domainFilter !== 'all' || !!buyerIdFilter;

  const clearFilters = () => {
    setSearchTerm('');
    setDomainFilter('all');
    if (buyerIdFilter) navigate('/marketplace');
  };
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder={viewMode === 'recommended' ? "Search within recommendations..." : "Search projects, clients..."}
