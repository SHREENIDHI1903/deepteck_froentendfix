...
export default function MessagesPage() {
  ...
  const [voiceModerationResult, setVoiceModerationResult] =
  ...
  const handleSendMessage = async () => {
    ...
    if (!modResult.isAllowed) {
      ...
      // Log the attempt
      const primaryViolation = modResult.violations[0];
      try {
        await adminApi.logCircumvention({
          chatId: selectedConversation,
          detectedType: primaryViolation ? primaryViolation.type : 'unknown',
          detectedValue: primaryViolation && primaryViolation.matches ? [...new Set(primaryViolation.matches)].join(', ') : 'unknown',
          content: messageText.trim()
        }, token || '');
      } catch (err) {
        console.error("Failed to log circumvention:", err);
      }
  ...
  async function generateWaveform(file: File, samples = 60): Promise<number[]> {
                                ...
                                <div className="mt-2 space-y-2">
                                  {message.attachments.map((a: any) => {
                                    if (a.mimeType?.startsWith("audio/")) {
                                      return <VoiceMessage key={a.id} attachmentId={a.id} />;
                                    }
                    ...
                    <div className="flex items-center gap-3 rounded-lg border p-3 bg-muted">
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          const result = await sendVoice.mutateAsync({
                            chatId: selectedConversation,
                            file: pendingVoice,
                          });

                          if (result.status === "rejected") {
                            setVoiceModerationResult(result.moderation);
                            return;
                          }
